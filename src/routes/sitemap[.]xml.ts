    ...
    handlers: {
      GET: async () => {
        const entries: SitemapEntry[] = [
          ...
          ...equipment.map((e) => ({ path: `/equipamentos/${e.slug}`, changefreq: "weekly" as const, priority: "0.7" })),
          ...companies.map((c) => ({ path: `/empresas/${c.slug}`, changefreq: "weekly" as const, priority: "0.6" })),
        ];
